...
Displays:
  - Current epic and task progress
...
render_task_progress() {
    ...
    # Parse epic statuses using awk (portable: BSD awk + GNU awk)
    local epic_data
    epic_data=$(awk '
        /epic_id:/ { epic_id = $2 }
    ...
    if [[ -z "$epic_data" ]]; then
        echo -e "  ${COLOR_DIM}(no epics found)${COLOR_RESET}"
        echo ""
        return
    fi
    ...
    while IFS='|' read -r eid etitle estatus; do
        ...
        # Count tasks per epic (portable: avoid grep -c || echo double-output)
        local task_total task_done
        task_total=$(grep -c "id: ${eid/EPIC-/TASK-}" "$todos_file" 2>/dev/null) || task_total=0
        task_done=$(awk -v prefix="${eid/EPIC-/TASK-}" '
