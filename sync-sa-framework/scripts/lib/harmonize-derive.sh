...
derive_agents_md() {
    ...
    # Extract project name from first h1 heading, skipping fenced code blocks
    local project_name
    project_name=$(awk '
        /^[[:space:]]*(```|~~~)/ { in_fence = !in_fence; next }
        !in_fence && /^# / { sub(/^# /, ""); print; exit }
    ' "$claude_file")
...
EOF
    ...
    # These lists are sourced from the policy-sections manifest (single source
    # of truth) via markdown_merge.py. The literals below are the legacy
    # fallback, used only if the manifest or a Python interpreter is unavailable.
    # See docs/designs/vendor-neutral-core-prompt.md.
    ...
    local _mm_lib _py _ext _sub
    _mm_lib="$(dirname "${BASH_SOURCE[0]}")/markdown_merge.py"
    _py="$(command -v python3 || command -v python || true)"
    if [[ -n "$_py" && -f "$_mm_lib" ]]; then
        _ext="$("$_py" "$_mm_lib" manifest --agents-extract 2>/dev/null || true)"
        _sub="$("$_py" "$_mm_lib" manifest --agents-subsections 2>/dev/null || true)"
        [[ -n "$_ext" ]] && sections_to_extract="$_ext"
        [[ -n "$_sub" ]] && sections_with_subsections="$_sub"
    fi
