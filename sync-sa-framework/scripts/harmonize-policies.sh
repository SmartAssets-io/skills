...
# Script location and library path. Resolve symlinks first: installers
# symlink this entry point (e.g. ~/.claude/scripts/harmonize-policies.sh),
# and lib/ only exists next to the real file.
_hp_source="${BASH_SOURCE[0]}"
while [ -L "$_hp_source" ]; do
    _hp_dir="$(cd -P "$(dirname "$_hp_source")" && pwd)"
    _hp_source="$(readlink "$_hp_source")"
    if [[ "$_hp_source" != /* ]]; then
        _hp_source="${_hp_dir}/${_hp_source}"
    fi
done
SCRIPT_DIR="$(cd -P "$(dirname "$_hp_source")" && pwd)"
unset _hp_source _hp_dir
