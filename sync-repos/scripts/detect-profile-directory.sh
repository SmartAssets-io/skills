...
# for Task/Epic/Story files (docs/ToDos.md, docs/UserStories.md, etc.) instead
...
#   PROFILE_ANCESTOR_DEPTH=N   When peer-pattern detection fails, walk up to N
#                              ancestor levels. At each ancestor: (a) if the
#                              ancestor itself is a git repo containing
#                              docs/ToDos.md, treat it as the workspace profile;
#                              (b) check the same 7 peer patterns at that
...
#
#   # Walk up to 3 ancestor levels looking for a workspace profile
#   PROFILE_ANCESTOR_DEPTH=3 detect-profile-directory.sh /path/to/orphan-repo
#
...
Environment:
  PROFILE_DEBUG=1            Enable debug output
  PROFILE_ANCESTOR_DEPTH=N   When no peer profile is found, walk up N
                             ancestor levels searching for a workspace
                             profile (an ancestor git repo with
                             docs/ToDos.md) or peer patterns at each
                             ancestor level. Default 0 (disabled).
...
find_profile_directory() {
    ...
    # Optional ancestor walk for orphan repos that have no peer profile.
    # Walks up PROFILE_ANCESTOR_DEPTH levels and at each level checks both
    # (a) whether the ancestor itself is a workspace profile (git repo with
    # docs/ToDos.md) and (b) the same 7 peer patterns at the ancestor's level.
    local max_depth="${PROFILE_ANCESTOR_DEPTH:-0}"
    if [[ "$max_depth" -gt 0 ]]; then
        local ancestor_path="$repo_path"
        local depth
        for ((depth = 1; depth <= max_depth; depth++)); do
            ancestor_path="$(dirname "$ancestor_path")"
            [[ "$ancestor_path" == "/" || -z "$ancestor_path" ]] && break

            local ancestor_name ancestor_parent
            ancestor_name="$(basename "$ancestor_path")"
            ancestor_parent="$(dirname "$ancestor_path")"

            debug "Ancestor walk depth=$depth path=$ancestor_path"

            # (a) Is the ancestor itself a workspace profile?
            if is_git_repo "$ancestor_path" && [[ -f "$ancestor_path/docs/ToDos.md" ]]; then
                local real_ancestor real_repo
                real_ancestor="$(cd "$ancestor_path" && pwd)"
                real_repo="$(cd "$repo_path" && pwd)"
                if [[ "$real_ancestor" != "$real_repo" ]]; then
                    debug "Found workspace profile (ancestor has docs/ToDos.md): $ancestor_path"
                    echo "$ancestor_path"
                    return 0
                fi
            fi

            # (b) Standard 7 peer patterns at the ancestor's level.
            local ancestor_patterns=(
                "${ancestor_parent}/${ancestor_name}_gitlab-profile"
                "${ancestor_parent}/${ancestor_name}-gitlab-profile"
                "${ancestor_parent}/gitlab-profile"
                "${ancestor_parent}/${ancestor_name}_github-profile"
                "${ancestor_parent}/${ancestor_name}-github-profile"
                "${ancestor_parent}/github-profile"
                "${ancestor_parent}/codeberg"
            )
            for pattern in "${ancestor_patterns[@]}"; do
                debug "Ancestor depth=$depth peer-check: $pattern"
                if [[ -d "$pattern" ]] && is_profile_directory "$pattern"; then
                    local real_pattern real_repo
                    real_pattern="$(cd "$pattern" && pwd)"
                    real_repo="$(cd "$repo_path" && pwd)"
                    if [[ "$real_pattern" != "$real_repo" ]]; then
                        debug "Found ancestor-peer profile: $pattern"
                        echo "$pattern"
                        return 0
                    fi
                fi
