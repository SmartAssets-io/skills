...
find_source_path() {
    ...
    # Auto-detect: look for gitlab-profile (script-relative location first)
    local candidates=(
        "${SCRIPT_DIR:-}/../.."
        "${SA_GITLAB_PROFILE:-}"
        "$(pwd)/gitlab-profile"
...
discover_repos() {
    ...
    # Re-add the target itself if it qualifies as a local umbrella
    # (no-remote git repo containing nested git repos). The leaf-only and
    # remote filters above strip these by design; this brings them back
    # for the workspace-subset code path in process_repository, which
    # re-detects via is_local_umbrella() since this function runs in a
    # subshell of the caller.
    if type -t is_local_umbrella &>/dev/null; then
        local target_abs
        target_abs=$(realpath "$target" 2>/dev/null)
        if [[ -n "$target_abs" ]] && is_local_umbrella "$target_abs"; then
            local already=false r
            for r in "${repos[@]}"; do
                if [[ "$(realpath "$r" 2>/dev/null)" == "$target_abs" ]]; then
                    already=true
                    break
                fi
            done
            if [[ "$already" == false ]]; then
                repos=("$target_abs" "${repos[@]}")
            fi
        fi
    fi
