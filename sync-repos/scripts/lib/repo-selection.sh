...
#
# Find .multi-repo-selection.jsonc in the start directory only
# Falls back to .multi-repo-selection.json for backward compatibility
#
# Only the start directory is checked — ancestors are NOT walked. A config in
# a parent workspace applies only when commands are invoked from that
# workspace root. Running from a sub-workspace (e.g. EphemeralInfrastructure/
# inside an SA/ workspace) ignores the parent config so the sub-workspace's
# nested repos are discovered normally. To opt back in, cd to the workspace
# root, place a config in the sub-workspace, or use MULTI_REPO_ALL=true.
#
# Arguments:
...
find_selection_config() {
    ...
    # Prefer .jsonc, fall back to .json — start directory only.
    if [[ -f "$search_dir/.multi-repo-selection.jsonc" ]]; then
        if [[ -f "$search_dir/.multi-repo-selection.json" ]]; then
            echo "Warning: both .multi-repo-selection.jsonc and .multi-repo-selection.json exist in $search_dir; using .jsonc and ignoring .json" >&2
        fi
        REPO_SELECTION_CONFIG="$search_dir/.multi-repo-selection.jsonc"
        return 0
    fi
    if [[ -f "$search_dir/.multi-repo-selection.json" ]]; then
        REPO_SELECTION_CONFIG="$search_dir/.multi-repo-selection.json"
        return 0
    fi
...
#
# Arguments:
#   $1 - Directory to look in for the config file (no ancestor walk).
#        This is also the directory whose repos are counted/selected.
#        Typically the user's CWD or the workspace the caller is acting on.
#
# Sets:
...
#
# Check whether a path is a "local umbrella" — a workspace-style git repo
# that contains nested git repos and has no remote configured. These dirs
# are filtered out by the leaf-only + remote filters but represent a real
# place that benefits from CLAUDE/AGENTS/GEMINI guidance.
#
# Called twice during harmonization: once in discover_repos() to decide
# whether to re-add the target after filtering, and once in
# process_repository() to gate per-repo file handlers. Idempotent, so
# detecting at both points avoids passing state across the subshell
# boundary created by `repos=$(discover_repos ...)`.
#
# Arguments:
#   $1 - Path to candidate repo
#
# Returns:
#   0 if path qualifies as a local umbrella, 1 otherwise
#
is_local_umbrella() {
    local repo_path="$1"
    [[ -d "$repo_path/.git" ]] || return 1
    if has_git_remote "$repo_path"; then return 1; fi

    local nested
    nested=$(find "$repo_path" -mindepth 2 -maxdepth "${SCAN_DEPTH:-8}" \
        \( -name node_modules -o -name vendor -o -name __pycache__ \) -prune \
        -o -type d -name ".git" -print -prune 2>/dev/null | head -1)
    [[ -n "$nested" ]]
}
