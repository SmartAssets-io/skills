#!/usr/bin/env bash
#
# epic-hygiene.sh - Validate and analyze epic/task YAML in docs/*.md files
#
# This deterministic script:
# 1. Validates YAML syntax in task tracking files
# 2. Detects epics where all tasks are done (ready to archive)
# 3. Detects orphaned tasks (not assigned to any epic)
# 4. Detects stale work logs for completed epics
# 5. Outputs JSON for use by /epic-hygiene slash command
#
# Usage (run from any repo with docs/ToDos.md):
#   $SA_GITLAB_PROFILE/AItools/scripts/epic-hygiene.sh [--json] [--check] [--verbose]
#
# Options:
#   --json     Output results as JSON (default: human-readable)
#   --check    Exit with code 1 if any epics are ready to archive
#   --verbose  Show detailed parsing information
#
# Files analyzed:
#   - docs/ToDos.md          (active tasks and epics)
#   - docs/CompletedTasks.md (archived epics)
...
# Source epic-parser library for robust YAML parsing
EPIC_PARSER="${LIB_DIR}/epic-parser.sh"
if [[ -f "$EPIC_PARSER" ]]; then
    # shellcheck source=lib/epic-parser.sh
    source "$EPIC_PARSER"
else
    echo "Error: epic-parser.sh library not found at $EPIC_PARSER" >&2
    exit 1
fi
...
# Note: extract_yaml_blocks and parse_yaml_fields are now provided by epic-parser.sh library
# The library provides robust nested YAML parsing via parse_epics(), derive_epic_status(), etc.

# Analyze ToDos.md for epic status
# Uses epic-parser.sh library for robust nested YAML parsing
analyze_todos() {
    ...
    # Use epic-parser library to parse all epics with nested tasks
    local epics_json
    epics_json=$(parse_epics "$todos_file")

    if [[ -z "$epics_json" ]] || [[ "$epics_json" == "[]" ]]; then
        log_verbose "No epics found in $todos_file"
        epics_json="[]"
    fi

    # Analyze epics using jq to find those ready to archive
    # An epic is ready to archive if:
    # 1. It has status: complete (explicitly marked), OR
    # 2. All its tasks have status: complete
    local analysis_json
    analysis_json=$(echo "$epics_json" | jq '
        . as $epics |
        {
            epics_total: ($epics | length),
            tasks_total: ([$epics[].tasks[]] | length),
            epics_ready_to_archive: [
                $epics[] |
                select(
        ...
        . + {needs_action: ((.epics_ready_to_archive | length) > 0)}
    ')

    # Extract completed epic info for work log analysis
    local completed_epics_json
    completed_epics_json=$(echo "$analysis_json" | jq '.epics_ready_to_archive')

    local completed_task_ids_json
    completed_task_ids_json=$(echo "$analysis_json" | jq '[.epics_ready_to_archive[].task_ids[]]')

    # Analyze work logs for stale entries
    log_verbose "Analyzing work logs for stale entries..."
    local stale_work_logs
    stale_work_logs=$(analyze_work_logs "$completed_epics_json" "$completed_task_ids_json")
    ...
    local epics_count tasks_count ready_count stale_logs_count
    epics_count=$(echo "$analysis_json" | jq -r '.epics_total')
    tasks_count=$(echo "$analysis_json" | jq -r '.tasks_total')
    ready_count=$(echo "$analysis_json" | jq -r '.epics_ready_to_archive | length')
    stale_logs_count=$(echo "$analysis_json" | jq -r '.stale_work_logs | length')

    # Log verbose info about each epic
    if [[ "$VERBOSE" == true ]]; then
        echo "$epics_json" | jq -r '.[] | "Found epic: \(.epic_id) (\(.status))"' | while read -r line; do
            log_verbose "$line"
        done
    fi
    ...
    if [[ "$OUTPUT_JSON" == true ]]; then
        ...
        if [[ "$ready_count" -gt 0 ]]; then
            echo -e "${GREEN}Epics ready to archive:${NC}"
            echo "$analysis_json" | jq -r '.epics_ready_to_archive[] | "  - \(.epic_id): \(.title) (\(.task_count) tasks)"'
            echo ""
        else
            echo "No epics ready to archive."
            echo ""
        fi
        ...
        if [[ "$orphan_count" -gt 0 ]]; then
            echo -e "${YELLOW}Orphan tasks (no epic assigned):${NC}"
        ...
        echo -e "${BLUE}Reminder:${NC} Run /epic-hygiene when all tasks in an epic are done"
        echo "          to archive completed epics to docs/CompletedTasks.md"
        echo ""
    fi
...
# Analyze work logs and find stale ones (related to completed epics/tasks)
# Returns JSON array of stale work log info
analyze_work_logs() {
    local work_logs_dir="${DOCS_DIR}/work-logs"
    local completed_epics_json="$1"
    ...
    while IFS= read -r -d '' log_file; do
        ...
        # Method 2: Check if filename matches a completed epic pattern (task-EPIC-XXX-*)
        if [[ "$is_stale" == false ]] && [[ "$completed_epics_json" != "[]" ]]; then
            # Extract epic ID from filename if present
            local filename_epic=""
            if [[ "$filename" =~ (EPIC-[0-9]+) ]]; then
                filename_epic="${BASH_REMATCH[1]}"
                if echo "$completed_epics_json" | jq -e --arg eid "$filename_epic" '.[] | select(.epic_id == $eid)' > /dev/null 2>&1; then
                    is_stale=true
                    stale_reason="epic_completed"
                fi
            fi
        fi
